...
export function TweakToggleGroup({ label, value, options, onChange }) {
  return (
    <TweakRow label={label}>
      <div className="twk-seg">
        {options.map((o) => (
          <button key={o} type="button" onClick={() => onChange(o)}
                  style={{ fontWeight: o === value ? 600 : 400 }}>{o}</button>
        ))}
      </div>
    </TweakRow>
  );
