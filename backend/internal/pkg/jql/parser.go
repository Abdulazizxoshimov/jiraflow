...
// Query is the parsed JQL result.
type Query struct {
	Conditions []Condition
	Logic      []string // AND | OR between consecutive conditions (len = len(Conditions)-1)
	OrderBy    []OrderClause
}
...
func (p *parser) expect(k tokenKind) (token, error) {
	t := p.consume()
	if t.kind != k {
		return t, fmt.Errorf("jql: expected token %d got %q", k, t.val)
	}
	return t, nil
}

// Parse parses a JQL string and returns a Query.
...
func (p *parser) parseCondition() (Condition, error) {
	...
	if err != nil {
		return Condition{}, fmt.Errorf("jql: expected field name: %w", err)
	}
	...
	case tokIsNot:
		operator = "IS NOT"
	default:
		return Condition{}, fmt.Errorf("jql: unknown operator %q", op.val)
	}
